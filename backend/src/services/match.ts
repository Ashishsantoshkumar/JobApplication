...
function generateExplanation(resume: Resume, job: Job): MatchExplanation {
...
export async function rankJobsForResume(resumeId: string): Promise<MatchedJob[]> {
  // Fetch resume
  ...
  if (db.isUsingFallback()) {
    const fallbackDb = (db as any).fallbackData;
    targetResume = fallbackDb.resumes.find((r: any) => r.id === resumeId) || null;
  } else {
    try {
      const res = await db.query('SELECT id, extracted_text, parsed_json, resume_embeddings::text as resume_embeddings FROM resumes WHERE id = $1', [resumeId]);
  ...
  if (!db.isUsingFallback() && targetResume.resume_embeddings) {
    try {
